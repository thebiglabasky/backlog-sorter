...
import { clearCache, clearIssuesCache, clearScoringCache, getCacheMetadata, getScoringCacheMetadata, loadCachedScoredIssues } from './cache-manager.js';
...
async function main() {
    ...
    try {
        ...
        const issueFetcher = new IssueFetcher(client, config, !args.noCache, args.forceRefresh);
        // Fetch issues
        const issues = await issueFetcher.fetchIssues();
        if (issues.length === 0) {
        ...
        const issueScorer = new IssueScorer(config, !args.noCache && !args.noScoringCache, args.forceRefresh);
        // Score issues
