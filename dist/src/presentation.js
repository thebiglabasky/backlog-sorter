 ...
 */
export function displayRankingChanges(changes, maxChangesToShow = 10) {
    if (changes.length === 0) {
 ...
 */
export function displaySortedIssuesWithChanges(sortedIssues, changes, maxToShow = 20) {
 ...
 */
export function displayIssueWithChange(item, index, changeMap) {
    ...
    console.log(`${chalk.cyan(`${index + 1}.`)} [${chalk.bold(item.issue.identifier)}] ${item.issue.title} - Score: ${chalk.green(item.finalScore.toFixed(1))} ${changeIndicator}`);
}
/**
 * Display detailed scoring results
 */
export function displayDetailedResults(sortedIssues) {
 ...
 */
export function displayStatistics(sortedIssues) {
    ...
    const avgValueScore = sortedIssues.reduce((sum, item) => sum + item.valueScore, 0) / sortedIssues.length;
    const avgComplexityScore = sortedIssues.reduce((sum, item) => sum + item.complexityScore, 0) / sortedIssues.length;
    // Count priorities
